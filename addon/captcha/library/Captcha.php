...
class Captcha{
	...
	public function create(){
		...
		if($this->use_zh){
			...
			for($i = 0; $i<$this->length; $i++){
				$code[$i] = iconv_substr($this->zh_set, floor(mt_rand(0, mb_strlen($this->zh_set, 'utf-8')-1)), 1, 'utf-8');
			}
		}else{
			for($i = 0; $i<$this->length; $i++){
				$code[$i] = $this->code_set[mt_rand(0, strlen($this->code_set)-1)];
			}
		}
	...
	public function entry(){
		...
		// 图片宽(px)
		$this->image_w || $this->image_w = $this->length*$this->font_size*1.5+$this->length*$this->font_size/2;
		// 图片高(px)
		$this->image_h || $this->image_h = $this->font_size*2.5;
		// 建立一幅 $this->image_w x $this->image_h 的图像
		...
		// 验证码使用随机字体
		$ttfPath = __DIR__.'/../assets/'.($this->use_zh ? 'zhttfs' : 'ttfs').'/';
		...
		if($this->use_zh){
			...
			for($i = 0; $i<count($code); $i++){
				imagettftext($this->im, $this->font_size, mt_rand(-40, 40), $this->font_size*($i+1)*1.5, $this->font_size+mt_rand(10, 20), $this->color, $this->fontttf, $code[$i]);
			}
		}else{
			for($i = 0; $i<count($code); $i++){
				$codeNX += mt_rand($this->font_size*1.2, $this->font_size*1.6);
				imagettftext($this->im, $this->font_size, mt_rand(-40, 40), $codeNX, $this->font_size*1.6, $this->color, $this->fontttf, $code[$i]);
			}
		}
	...
	private function writeCurve(){
		...
		$T = mt_rand($this->image_h, $this->image_w*2); // 周期
		$w = (2*M_PI)/$T;
		
		$px1 = 0; // 曲线横坐标起始位置
		$px2 = mt_rand($this->image_w/2, $this->image_w*0.8); // 曲线横坐标结束位置
	...
	private function writeNoise(){
		$code_set = '2345678abcdefhijkmnpqrstuvwxyz';
		for($i = 0; $i<10; $i++){
			//杂点颜色
			$noiseColor = imagecolorallocate($this->im, mt_rand(150, 225), mt_rand(150, 225), mt_rand(150, 225));
			for($j = 0; $j<5; $j++){
				// 绘杂点
				imagestring($this->im, 5, mt_rand(-10, $this->image_w), mt_rand(-10, $this->image_h), $code_set[mt_rand(0, 29)], $noiseColor);
			}
		}
	}
	...
	private function background(){
		...
		@imagecopyresampled($this->im, $bgImage, 0, 0, 0, 0, $this->image_w, $this->image_h, $width, $height);
