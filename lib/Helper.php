...
class Helper {
	 ...
	 * @return string
	 */
	public static function strFieldJuli($lat, $lng){
	 ...
	 * @return array 多维数组
	 */
	public static function sortArrayRecursio($data, $option = []){
	 ...
	 * @return string 签名字符串
	 */
	public static function sign($data, $algo = 'md5'){
		if(is_array($data)){
			ksort($data);
		}
	 ...
	 */
	public static function birthExt($birth){
	 ...
	 * @return string
	 */
	public static function htmlTagsStrip($str, $tags, $content = 0){
		if($content){
			$html = array();
			foreach($tags as $tag){
				$html[] = '/(<'.$tag.'.*?>[\s|\S]*?<\/'.$tag.'>)/';
	 ...
	 * @return string
	 */
	public static function htmlTagsRetain($str, $tags = '<img><a>'){
		$search = array(
	 ...
	 * 手机号格式验证
	 */
	public static function isMobile($mobile, $type = array()){
		$regular = [
	 ...
	 * @return string
	 */
	public static function convertHump($name, $type = 0, $ucfirst = true){
		if($type){
