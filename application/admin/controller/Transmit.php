...
class Transmit extends \app\common\controller\BaseAdmin{
	...
	public function upload(){
		...
		if(!$result) return $this->ajaxReturn(['code' => 100, 'msg' => $s_transmit->getError()]);
		//进行上传
		$data = $s_transmit->upload();
		if(!$data) return $this->ajaxReturn(['code' => 100, 'msg' => $s_transmit->getError()]);
		if(count($data)==1) $data = $data[0]; //单文件上传，返回参数变更
		/* 上传成功 */
	...
	public function download($name = '', $type = ''){
		...
		if(!$result) $this->error($s_transmit->getError(), url('admin/index/index'));
	}
	...
	public function upload_editor($type = ''){
		if(!$type) return $this->ajaxReturn(['err' => '类型不能为空', 'msg' => '']);
		//校验
		$s_transmit = service('Transmit');
		$result     = $s_transmit->checkType($type);
		if(!$result) return $this->ajaxReturn(['err' => $s_transmit->getError(), 'msg' => '']);
		//进行上传
		$files = $this->request->file('filedata'); //获取表单上传文件
		$data  = $s_transmit->upLocalFile($files, $type);
		if(!$data) return $this->ajaxReturn(['err' => $s_transmit->getError(), 'msg' => '']);
		//上传成功
