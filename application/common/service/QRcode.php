...
class QRCode extends Base{
	 ...
	 * @return resource
	 */
	public function create($string, $logo = false){
		//生成二维码图片 无logo
		$image = \PHPQRcode\QRcode::png($string, false, $this->config['level'], $this->config['size'], $this->config['margin']);
		//带logo
		if($logo){
			$image = $this->createLogo($image, $logo);
		}
		return $image;
	}
	 ...
	 * @return resource
	 */
	public function createLogo($image, $logo){
		//二维码尺寸
		$image_width  = imagesx($image);
		$image_height = imagesy($image);
		
		/* 处理圆角图片 */
		$corner = $this->createRadiusCorner();
		//圆角图尺寸
		$corner_width  = imagesx($corner);
		$corner_height = imagesy($corner);
		//调整后尺寸
		$corner_width_after  = $image_width*$this->config['logo_size'];
		...
		//logo调整后尺寸
		$scale             = ($logo_width>$logo_height ? $logo_width : $logo_height)/($image_width*$this->config['logo_border']); //缩放比例
	 ...
	 * @return resource
	 */
	public function createRadiusCorner($image_width = 300, $image_height = 300, $radius = 30){
		/* 创建图片 */
